...
${COMPOSE} exec -T timeseries_table bash -lc "target/release/tstable create --table '${TABLE_DIR_BULK}' --time-column '${TIME_COLUMN}' --bucket 1s"
...
start=$(now_ms)
${COMPOSE} exec -T timeseries_table bash -lc "target/release/tstable append --table '${TABLE_DIR_BULK}' --parquet '${bulk_path}'"
...
${COMPOSE} exec -T timeseries_table bash -lc "target/release/tstable create --table '${TABLE_DIR}' --time-column '${TIME_COLUMN}' --bucket 1s"
...
for file in "${daily_files[@]}"; do
  ...
  start=$(now_ms)
  ${COMPOSE} exec -T timeseries_table bash -lc "target/release/tstable append --table '${TABLE_DIR}' --parquet '${path}'"
  elapsed=$(( $(now_ms) - start ))
...
for q in q1_time_range q2_agg q3_filter_agg q4_groupby q5_date_trunc; do
  ...
  start=$(now_ms)
  ${COMPOSE} exec -T timeseries_table bash -lc "target/release/tstable query --table '${TABLE_DIR}' --sql \"${sql_escaped}\" --max-rows 0 >/dev/null"
  elapsed=$(( $(now_ms) - start ))
