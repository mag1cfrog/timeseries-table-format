...
${COMPOSE} exec -T timeseries_table bash -lc "cargo build -p timeseries-table-cli --release"
...
start=$(now_ms)
${COMPOSE} exec -T timeseries_table bash -lc "target/release/timeseries-table-cli append --table '${TABLE_DIR}' --parquet '${bulk_path}'"
...
for q in q1_time_range q2_agg q3_filter_agg q4_groupby q5_date_trunc q6_date_bin; do
  ...
  start=$(now_ms)
  ${COMPOSE} exec -T timeseries_table bash -lc "target/release/timeseries-table-cli query --table '${TABLE_DIR}' --sql \"${sql_compact}\" --max-rows 0 >/dev/null"
  elapsed=$(( $(now_ms) - start ))
