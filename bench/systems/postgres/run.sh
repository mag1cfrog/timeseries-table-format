...
CSV_OUT="${RESULTS_RUN_DIR}/postgres.csv"
ensure_csv_header "$CSV_OUT"

for _ in {1..30}; do
  if ${COMPOSE} exec -T postgres pg_isready -U bench >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
...
start=$(now_ms)
${COMPOSE} exec -T postgres psql -U bench -d bench -c "\\copy trips FROM '${bulk_path}' WITH (FORMAT csv, HEADER true)"
...
for file in "${daily_files[@]}"; do
  ...
  start=$(now_ms)
  ${COMPOSE} exec -T postgres psql -U bench -d bench -c "\\copy trips FROM '${path}' WITH (FORMAT csv, HEADER true)"
  elapsed=$(( $(now_ms) - start ))
  emit_row "$CSV_OUT" "postgres" "daily_append" "$rel" "$rows" "$bytes" "$elapsed" "$CPU_LIMIT" "$MEM_LIMIT" ""
done
