...
echo "==> QuestDB: data prep"
if [[ ! -f "${MANIFEST_CSV}" ]]; then
  "${ROOT_DIR}/bench/scripts/run_data_prep.sh"
else
  echo "==> QuestDB: data prep already complete"
fi

echo "==> QuestDB: start services"
pushd "${ROOT_DIR}/bench" >/dev/null
  docker compose -f compose.yml up -d --build questdb
popd >/dev/null
echo "==> QuestDB: services up"

echo "==> QuestDB: run"
"${ROOT_DIR}/bench/systems/questdb/run.sh"
echo "==> QuestDB: done"

echo "==> QuestDB: stop services"
pushd "${ROOT_DIR}/bench" >/dev/null
  docker compose -f compose.yml stop questdb
popd >/dev/null
